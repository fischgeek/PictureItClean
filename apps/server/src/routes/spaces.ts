...
spacesRouter.post("/areas/:areaId/spaces", requireRole("area", "areaId", "editor"), (req, res) => {
  ...
  const space = repos.spaces.create({ areaId: req.params.areaId, name, sortOrder: sortOrder ?? existing.length });
...
spacesRouter.patch("/spaces/:spaceId", requireRole("space", "spaceId", "editor"), (req, res) => {
  const { name, sortOrder } = req.body ?? {};
  res.json(repos.spaces.update(req.params.spaceId, { name, sortOrder }));
});
